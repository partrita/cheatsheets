...
2007년 구글에서 개발된 정적 타입 컴파일 언어. 간결한 구문, 가비지 컬렉션, CSP 스타일의 동시성 프로그래밍 지원이 특징임.

- 설치 확인: `go version`
- 파일 확장자: `.go`
- 주요 환경 변수:
  - `GOROOT`: Go 설치 디렉토리
  - `GOPATH`: 작업 공간 (최신 버전은 모듈 기반 관리 권장)

= 2. 변수와 상수

== 변수 (Variables)
- 선언 및 할당: `var a int = 10`
- 타입 추론: `var i = 1`
- 짧은 선언: `i := 1` (함수 내에서만 가능)
- 다중 선언: `var i, j int = 1, 2`

== 상수 (Constants)
- 선언: `const c int = 10`
- `iota`: 순차적 값 부여 (0부터 시작)
  ```go
  const (
      Apple = iota // 0
      Grape        // 1
