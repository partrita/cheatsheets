...
- CI (Confidence Interval, 신뢰구간): 동일 연구를 반복할 때 95% 확률로 나타나는 값의 범위. 신뢰구간이 겹치지 않는 차이가 유의미 하다.
- DLT (Dose-Limiting Toxicity, 용량 제한 독성): 투여 후 발생하는 심각한 부작용으로 인해, 그 이상의 용량 증량이 불가능하다고 판단되는 독성 수준. MTD 결정의 직접적인 기준이 됨.
- EHR(Electronic Health Record): 병원 정보시스템에 저장된 환자의 진단, 처치, 약 처방, 검사 결과 등을 포함한 디지털 기록.
