...
Go 프로그래밍을 시작하기 위해 Go 공식 웹사이트인 해당 OS 버젼의 Go를 다운로드하여 설치한다. 윈도우즈에 Go를 설치하기 위해서는 MSI파일을 다운받아 실행하면 되는데, Go는 디폴트로 `C:\Program Files\Go` (옛 버전은 `C:\go`) 폴더에 설치되며, MSI가 `C:\Program Files\Go\bin`을 System PATH 환경변수를 추가한다. Go를 설치하고 해당 설치 디렉토리 밑에 bin 디렉토리를 보면 `go.exe` 파일이 있는데, 이 컴파일러로 go 프로그램을 컴파일하거나 실행할 수 있다. Go 프로그램은 파일 확장자 `.go` 를 갖는다. Go가 설치되었으면, 아래와 같은 명령으로 Go 버전을 간단하게 체크해 볼 수 있다.
...
- GOPATH:
  - Go 프로젝트의 홈 디렉토리 역할을 하는 곳으로, 윈도우즈의 경우 디폴트로 `%USERPROFILE%\go` 폴더이며, 리눅스의 경우 `$HOME/go` 폴더이다. GOPATH/pkg 폴더는 3rd Party 패키지 및 사용자 정의 패키지 등을 저장하는 곳이고, GOPATH/bin 디렉토리는 3rd Party 실행파일 및 사용자 프로젝트의 go 실행파일을 저장하는 곳이다. 사용자 프로젝트에서 go install을 실행하면 GOPATH/bin 폴더에 저장된다.
