...
데이터 과학은 데이터를 통한 질문 해결 과정이지만, 데이터 정리(wrangling)에 대부분의 시간이 소요됩니다. 이를 돕기 위해 R에서는 Tidyverse(Tidyverse)라는 패키지 모음이 개발되었습니다. Tidyverse는 공통된 철학과 구조를 공유하며, 이 과정을 통해 깔끔한 데이터(tidy data) 개념과 Tidyverse 활용법을 익히는 것이 목표입니다. 이 과정은 R 프로그래밍 기본 지식을 전제로 하며, R에 익숙하지 않다면 선행 학습을 권장합니다.

Tidyverse를 사용하기 전에 먼저 설치해야 합니다.
```r
...
팩터로 작업하는 것을 더 간단하게 만들기 위해, 우리는 핵심 Tidyverse 패키지인 `forcats` 패키지를 활용할 것입니다. `forcats` 내의 모든 함수는 `fct_`로 시작하여 찾아보기 쉽고 기억하기 쉽습니다. 이전과 마찬가지로 사용 가능한 함수를 보려면 RStudio 콘솔에 `?fct_`를 입력하면 됩니다. 가능한 모든 `forcats` 함수가 포함된 드롭다운 메뉴가 나타날 것입니다.
...
Tidyverse는 R을 더 쉽게 사용할 수 있게 해주는 패키지들의 묶음입니다. 이 패키지들은 모두 함께 작동하도록 설계되었기 때문입니다. 대부분의 "타이디" 함수는 다음과 같은 이유로 서로 잘 작동합니다.

- 데이터프레임을 입력으로 받습니다.
- 데이터프레임을 출력으로 반환합니다.

분석에서 Tidyverse 패키지를 모두 사용하지 않을 수도 있지만, 대부분의 분석 시작 시 모두 로드하여 표준 도구 세트를 사용할 수 있습니다. 모두 로드하려면 다음을 사용하십시오.
...
Tidyverse에 있는 `dplyr` 패키지는 복잡한 작업을 수행하기 위해 쉽게 결합할 수 있는 소수의 간단한 동사를 제공하여 이를 더 쉽게 만듭니다. 각 동사는 현재 데이터를 가져와 변경한 다음 반환합니다. 가장 일반적으로 사용될 동사는 다음과 같습니다.
...
`dplyr` 및 기타 Tidyverse 패키지는 R 구문의 규칙을 구부려서 `survey1$group`, `survey1$time`처럼 매번 데이터프레임 이름을 모두 입력할 필요 없이 `group` 및 `time`과 같은 열 이름만 입력할 수 있도록 합니다. Tidyverse 함수 호출 내에서는 열 이름만 입력하면 됩니다. 예를 들면:
