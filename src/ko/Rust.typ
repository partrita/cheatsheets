...
- *섀도잉 (Shadowing)*: `let x = x + 1;`과 같이 같은 이름의 변수를 다시 선언하여 이전 변수를 덮어씁니다(가림).
...
- *반복문 (Loops)*:
  - `loop { ... }`: 무한 루프를 실행하며, `break` 뒤에 표현식을 두어 값을 반환할 수 있습니다.
...
- `match`: 모든 경우의 수를 철저히 검사하는 패턴 매칭 구문입니다.
  ```rust
  match option_value {
...
= 제네릭(Generics), 트레잇(Traits), 라이프타임(Lifetimes)
...
- `Arc<T>`: 여러 스레드 간에 소유권을 안전하게 공유하는 원자적 참조 카운팅 스마트 포인터입니다.
...
== 패키지 관리 및 Cargo 활용
- `Cargo.toml`로 의존성, 빌드 프로필(Release/Debug), 프로젝트 메타데이터를 통합 관리합니다.
- *크로스 컴파일*: `rustup target add`와 `cargo build --target` 명령으로 다양한 대상 플랫폼용 바이너리를 빌드합니다.
