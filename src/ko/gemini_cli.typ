...
- *YOLO 모드 활용*: 신뢰할 수 있는 반복 작업 시 `-y` 옵션으로 사용자 승인 단계를 건너뜁니다.
- *세션 재개*: 중단된 세션은 `gemini -r latest` 명령으로 마지막 작업 상태를 복원합니다.
- *비대화형 파이프라인*: 쉘 스크립트 등에서 `gemini -p "..." > output.md`와 같이 활용 가능합니다.
- *디렉토리 포함*: `--include-directories` 옵션으로 외부 경로를 현재 작업 영역에 추가합니다.
