...
- *클래스와 객체*: 클래스는 객체를 만들기 위한 설계도이며, 객체는 그 설계도를 바탕으로 생성된 실체(인스턴스)입니다.
  ...
  ```
- *핵심 4대 원칙*:
  1. *캡슐화*: 접근 제어자(`private`, `public` 등)로 필드 직접 접근을 제한하고 메서드로 제어합니다.
...
- *추상 클래스 (`abstract`)*: 하나 이상의 추상 메서드를 포함할 수 있는 미완성 클래스 (단일 상속).
...
- *스트림 API*: 컬렉션 데이터를 함수형 파이프라인 방식으로 가공·집계합니다.
  ```java
