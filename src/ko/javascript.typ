...
외부 함수가 종료된 후에도 자신이 선언되었을 때의 렉시컬 스코프(Lexical Scope)에 계속 접근할 수 있는 함수.
...
- 객체 리터럴: `quoted`와 `unquoted` 키 혼용 금지. 메서드 단축 표기(`method() {}`) 권장.
- Arrow Functions: 익명 함수와 `this` 바인딩 시 선호.
- Type Casting: `Number()`, `String()` 등 명시적 함수 사용.
- JSDoc: 타입 정보를 포함하여 충실히 작성.
