...
Julia는 고성능 수치 연산과 데이터 과학을 위해 개발된 언어로, 파이썬의 직관성과 C 언어 수준의 실행 속도를 동시에 제공합니다.

== 변수 정의

기본적으로 동적 타이핑 언어이며, 성능 최적화와 다중 디스패치를 위해 타입을 명시할 수 있습니다.
```julia
x = 10             # 정수형 (Int64)
y = "안녕하세요!"   # 문자열 (String)
z = 3.14           # 실수형 (Float64)
...
`using` 키워드로 모듈을 로드하고, `Pkg` 모듈로 패키지 의존성을 관리합니다.
```julia
