...
uv는 Rust로 작성된 초고속 파이썬 패키지 설치 및 환경 관리 도구입니다. 기존 `pip`, `pip-tools`, `venv`의 기능을 하나로 통합하여, 파이썬 개발 워크플로우의 작업 속도를 크게 단축합니다.
...
기존 `pip`과 호환되는 명령어 인터페이스로 가상 환경 상태를 점검합니다.
...
`uv`는 초고속 설치를 위해 적극적인 로컬 캐싱을 수행합니다. 캐시 정리 및 무결성 검증 명령어:
